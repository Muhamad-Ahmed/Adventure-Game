...
// Enemy variables
let enemies = ["Skeleton", "Zombie", "Grimclaw", "Hellfire", "Ghost"];
let enemyMaxHealth = 50;
let enemyHealth = 0;
let enemyMinHealth = 20;
let enemyCount = 3;
let enemyMaxDamage = 30;
// Player variables
let playerHealth = 50;
let playerMaxDamage = 50;
// Game variables
let medikitPopChance = 50; // 50% chance
let medikitHealAmount = 25;
let medikit = 1;
let running = true;
console.log(chalk.blue.bold.underline("!!!\tWelcome to the Dungeon!\t!!!\n"));
// Game loop
Game: while (running) {
    console.log(chalk.blue.bold.underline("#\tYou have to defeat 3 enemies to win the game.\t#\n"));
    // Set up the enemy
    enemyHealth = Math.round(Math.random() * (enemyMaxHealth - enemyMinHealth) + enemyMinHealth);
    ...
    while (playerHealth > 0 && enemyHealth > 0) {
        ...
        if (options.user === "Attack") {
            let damageDealt = Math.round(Math.random() * enemyMaxDamage) + 1; // Damage between 1 and 30
            let damageDone = Math.round(Math.random() * (playerMaxDamage - 25)) + 25; // Damage between 25 and 50
            playerHealth -= damageDealt;
            enemyHealth -= damageDone;
            console.log(chalk.bold.green(`###\t You strike the enemy for ${damageDone} damage. \t###\n`));
            console.log(chalk.bold.red(`###\t You have received ${damageDealt} damage in retaliation. \t###\n`));
            console.log(chalk.bold.green(`###\t Your health is ${playerHealth}. \t###\n`));
            console.log(chalk.bold.red(`###\t ${enemy} health is ${enemyHealth}. \t###\n`));
            if (playerHealth <= 0) {
                console.log(chalk.bold.italic.red.bgBlack("###\t You have fallen to your doom. \t###\n"));
                console.log(chalk.bold.italic.red.bgBlack(`###\t Game over. \t###\n`));
                running = false;
                break Game;
            }
            else if (enemyHealth <= 0) {
                console.log(chalk.bold.italic.underline.yellow(`###\t ${enemy} is dead. \t###\n`));
                enemyCount--;
                if (enemyCount === 0) {
                    console.log(chalk.bold.italic.cyan.bgBlack(`###\t\t Congratulations! You have won the Dungeon. \t\t###\n`));
                    break Game;
                }
                let medikitPop = Math.round(Math.random() * 100); // Number between 0 and 100
